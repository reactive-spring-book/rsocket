...
class Service {
	 ...
	 *
	 * @Override public Mono<Void> metadataPush(Payload payload) { var metadataUtf8 =
	 * payload.getMetadataUtf8(); var metadata =
	 * encodingUtils.decodeMetadata(metadataUtf8);// <1> return
	 * onMetadataPush(metadata);// <2> } }; return Mono.just(rs); }
	 *
	 * private Mono<Void> onMetadataPush(Map<String, Object> metadata) { var clientId =
	 * (String) metadata.get(Constants.CLIENT_ID_HEADER); var stringBuilder = new
	 * StringBuilder().append(System.lineSeparator()); stringBuilder//
	 * .append(String.format("(%s) %s", clientId, "---------------------------------"))//
	 * .append(System.lineSeparator()); metadata.forEach((k, v) -> stringBuilder//
	 * .append(String.format("(%s) %s", clientId, k + '=' + v))//
	 * .append(System.lineSeparator())); log.info(stringBuilder.toString()); return
	 * Mono.empty(); }
	 */
